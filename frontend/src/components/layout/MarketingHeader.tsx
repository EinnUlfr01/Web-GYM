...
export default function MarketingHeader() {
  ...
  const accountLinks: NavigationItem[] = [
    ...
    { to: dashboardRoute, activeTo: isAuthenticated && user ? dashboardRoute : '/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  ];
  ...
  const renderItem = (item: NavigationItem) => {
    ...
    return (
      <Link
        ...
        onClick={closeDrawer}
        className={`marketing-drawer-link ${isActive(item.activeTo || item.to) ? 'is-active' : ''}`}
        aria-current={isActive(item.activeTo || item.to) ? 'page' : undefined}
      >

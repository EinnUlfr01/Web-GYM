...
export default function ComplaintsPage({ role }: { role: Role }) {
  ...
  return <main className="mx-auto max-w-7xl space-y-6">
    ...
    {role === "buyer" && <form onSubmit={create} className="grid gap-5 rounded-2xl border border-slate-800 bg-slate-950/70 p-5 shadow-xl shadow-black/10 sm:p-6 md:grid-cols-2 lg:grid-cols-3">
      ...
      <div className="flex gap-3 rounded-xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm leading-6 text-amber-100 md:col-span-2 lg:col-span-3"><AlertTriangle className="mt-0.5 shrink-0 text-amber-300" size={18}/><p>Chỉ áp dụng trong 7 ngày từ khi giao và trước khi settlement PAID. Ngoài phạm vi này, vui lòng dùng <Link className="font-semibold text-amber-200 underline underline-offset-4 hover:text-white" to="/tickets">hỗ trợ thủ công</Link>; GymFit không hứa tự động hold hoặc refund.</p></div>
      <button className="btn-primary w-full sm:w-fit" type="submit">Gửi khiếu nại</button>
    </form>}

    {role === "admin" && <section aria-label="Bộ lọc khiếu nại" className="grid gap-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-5 sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1.3fr_1fr_1fr_auto]">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(300px,0.8fr)_minmax(0,1.2fr)]">
      <section aria-label="Danh sách khiếu nại" className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/55">
      ...
      <section aria-label="Chi tiết khiếu nại" className="min-h-[280px] rounded-2xl border border-slate-800 bg-slate-950/55 p-5 sm:p-6">
        {!selected ? <div className="grid min-h-56 place-items-center rounded-xl border border-dashed border-slate-800 text-center text-slate-400"><div><ClipboardList className="mx-auto mb-3 text-slate-600" size={30}/><p>Chọn một khiếu nại để xem chi tiết.</p></div></div> : <div className="space-y-5">
          ...
          {selected.settlementStatus === "PAID" && <p className="rounded-xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm leading-6 text-amber-100">Settlement đã PAID: chỉ manual support / carry-forward thủ công; không reopen, clawback hoặc tự áp adjustment.</p>}
          <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Mô tả</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-200">{selected.description}</p></div>
          {selected.adminDecisionReason && <p className="rounded-xl border border-blue-400/20 bg-blue-400/10 p-4 text-sm text-blue-100">Quyết định Admin: {selected.adminDecisionReason}</p>}
          ...
          {role === "admin" && <div className="flex flex-wrap gap-2 rounded-xl border border-slate-800 bg-slate-900/40 p-4">

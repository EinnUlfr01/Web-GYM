...
import { createDay, createProgramExercise, deleteDay, deleteProgramExercise, getProgram, listCoachExercises, reorderDays, reorderProgramExercises, updateDay, updateProgram, updateProgramExercise } from '../../services/coachWorkspaceApi';
...
export default function CoachProgramBuilderPage() {
  ...
  const load = () => {
    ...
    Promise.all([getProgram(Number(programId)), listCoachExercises({ page: 1, limit: 50, sort: 'name_asc' })])
      .then(([value, exerciseList]) => { setProgram(value); setExercises(exerciseList.items); setDirtyExercises({}); setSavedExercises({}); setDirtyDays({}); setSavedDays({}); })
  ...
  return <CoachPage title={program.name} description="Program Builder - edit only the current Coach-owned Program." backTo="/coach/workout-programs">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.2fr_1fr]"><section className="space-y-4">
      ...
      {days.map((day, dayIndex) => <article key={day.id} className="dashboard-panel"><div className="flex items-start justify-between gap-3"><div><p className="text-xs text-emerald-400">W{day.week_number} · D{day.day_number}</p><p className="text-sm text-slate-500">{day.exercises.length} exercise(s)</p></div><div className="flex gap-1"><button type="button" className="icon-button" aria-label={`Move ${day.title} up`} disabled={dayIndex === 0} onClick={() => void moveDay(dayIndex, -1)}><ArrowUp size={15}/></button><button type="button" className="icon-button" aria-label={`Move ${day.title} down`} disabled={dayIndex === days.length - 1} onClick={() => void moveDay(dayIndex, 1)}><ArrowDown size={15}/></button><button type="button" className="icon-button" aria-label={`Delete ${day.title}`} onClick={() => setConfirm({ kind: 'day', id: day.id })}><Trash2 size={16}/></button></div></div>
        <div className="mt-3 grid gap-2 md:grid-cols-[80px_80px_1fr]"><label className="text-xs text-slate-500">Week<input className={`${inputClass} mt-1`} type="number" min={1} max={104} value={day.week_number} onChange={event => updateDayDraft(day, 'week_number', Number(event.target.value))}/></label><label className="text-xs text-slate-500">Day<input className={`${inputClass} mt-1`} type="number" min={1} max={7} value={day.day_number} onChange={event => updateDayDraft(day, 'day_number', Number(event.target.value))}/></label><label className="text-xs text-slate-500">Title<input className={`${inputClass} mt-1`} value={day.title} onChange={event => updateDayDraft(day, 'title', event.target.value)}/></label></div><label className="mt-2 block text-xs text-slate-500">Description<textarea className={`${inputClass} mt-1 min-h-16`} value={day.description ?? ''} onChange={event => updateDayDraft(day, 'description', event.target.value)}/></label><div className="mt-3 flex flex-wrap items-center gap-3"><button type="button" className="secondary-button" disabled={savingDayId !== null} onClick={() => void saveDay(day)}>{savingDayId === day.id ? 'Saving...' : dirtyDays[day.id] ? 'Save Day' : savedDays[day.id] ? 'Saved' : 'No changes'}</button>{dirtyDays[day.id] && <span className="text-xs text-amber-200">Unsaved Day changes</span>}</div>

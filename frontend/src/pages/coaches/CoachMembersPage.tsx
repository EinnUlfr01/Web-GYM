...
export default function CoachMembersPage() {
  ...
  const load = () => {
    setLoading(true);
    setError('');
    listMembers({ q: q || undefined, page, limit: 20 })
      .then(result => {
        setItems(result.items);
        setTotal(result.total);
        setTotalPages(result.totalPages);
      })
      .catch(() => setError('Không thể tải Member trong scope.'))
  ...
  return <CoachPage title="My Members" description="Chỉ Member active được phân công cho Coach hiện tại mới xuất hiện." actions={<Link className="secondary-button" to="/coach/assignments/new">Gán Program</Link>}>
    ...
    {error ? <ErrorState message={error} retry={load} /> : loading ? <LoadingState /> : items.length === 0 ? <div className="dashboard-panel"><div className="panel-state">Chưa có Member active được phân công.</div></div> : <>

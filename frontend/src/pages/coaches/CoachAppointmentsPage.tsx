...
function AppointmentRow({ booking, onAction }: { booking: Booking; onAction: (id: number, status: Exclude<BookingStatus, 'pending'>) => Promise<Booking> }) {
  ...
  const run = async (status: Exclude<BookingStatus, 'pending'>) => {
    ...
    try { await onAction(booking.id, status); }
...
export default function CoachAppointmentsPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { setBookings(await getCoachAppointments({ limit: 100 })); } catch (reason: unknown) { setError(messageFor(reason)); } finally { setLoading(false); } }, []);
  ...
  const action = async (id: number, status: Exclude<BookingStatus, 'pending'>) => { const updated = status === 'confirmed' ? await confirmBooking(id) : status === 'completed' ? await completeBooking(id) : status === 'no_show' ? await markBookingNoShow(id) : await cancelCoachBooking(id); setBookings(current => current.map(booking => booking.id === id ? updated : booking)); return updated; };

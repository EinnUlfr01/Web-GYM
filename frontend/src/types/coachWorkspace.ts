...
export interface CoachProgram { id:number; name:string; description:string|null; goal:string; difficulty:string; duration_weeks:number; days_per_week:number; owner_coach_id:number; is_active:boolean; created_at:string; updated_at:string; day_count?:number; exercise_count?:number; days?:CoachProgramDay[]; }
...
export interface CoachSession { id:number; member_id:number; member_name:string; started_at:string; completed_at:string|null; status:string; workout_name:string; }
export interface CoachSessionHistoryItem { id:number; member_id:number; workout_id:number; started_at:string; completed_at:string|null; status:string; notes:string|null; workout_name:string; workout_description:string|null; setSummary:null; blockedReason:string; }
export interface CoachSessionDetail extends CoachSessionHistoryItem { exerciseSnapshot:unknown[]; }
export interface CoachProgressSession { id:number; started_at:string; completed_at:string|null; status:string; workout_name:string; }
export interface CoachProgress { completed_sessions:number; total_duration:number; training_volume:number|null; completion_rate:number|null; recent_sessions:CoachProgressSession[]; exercise_history:unknown[]; blockedReason:string; dataSources:{legacySessions:boolean;setLogs:boolean;memberProgressFlow:boolean}; }

...
export interface CoachSession { id:number; member_id:number; workout_id:number; started_at:string; completed_at:string|null; status:string; notes:string|null; workout_name:string; setSummary:null; blockedReason:string; }
export interface CoachProgress { completed_sessions:number; total_duration:number; training_volume:number|null; completion_rate:number|null; recent_sessions:CoachSession[]; exercise_history:unknown[]; blockedReason:string; dataSources:{legacySessions:boolean;setLogs:boolean;memberProgressFlow:boolean}; }

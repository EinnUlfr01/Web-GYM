...
export const getPublic=async(req:Request,res:Response,next:NextFunction)=>{try{const page=Math.max(1,Number(req.query.page)||1),limit=Math.min(100,Math.max(1,Number(req.query.limit)||12));const data=await shopsService.publicDetail(req.params.shopSlug,page,limit);res.json({success:true,data:data.shop,products:data.products,pagination:{page,limit,total:data.total,pages:Math.ceil(data.total/limit)}});}catch(e){next(e);}};

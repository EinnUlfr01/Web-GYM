...
const assertTimeZone = (timeZone: string): void => {
  ...
  catch { throw new AppError(409, 'Assignment has an invalid schedule timezone'); }
};
...
async function assignmentForMember(memberId: number) {
  const result = await query(
    `SELECT TOP 1 a.id,a.member_id,a.program_id,a.coach_id,a.start_date,a.end_date,a.status,a.schedule_timezone,a.note,
...
export async function getCurrent(memberId: number) {
  ...
  const [program, schedules, active] = await Promise.all([
    programForMember(memberId, Number(assignment.program_id)),
    query(`SELECT TOP 20 s.id,s.assignment_id,s.program_day_id,s.scheduled_date,s.status,s.created_at,s.updated_at,d.week_number,d.day_number,d.title AS day_title FROM dbo.CoachProgramSchedules s JOIN dbo.CoachProgramAssignments a ON a.id=s.assignment_id AND a.member_id=@memberId JOIN dbo.WorkoutProgramDays d ON d.id=s.program_day_id WHERE s.assignment_id=@assignmentId AND s.status<>N'CANCELLED' ORDER BY s.scheduled_date,s.id`, { memberId, assignmentId: assignment.id }),
  ...
  return { assignment, program, upcomingSchedules: schedules.recordset.map(item => ({ ...item, scheduled_date: datePart(item.scheduled_date) })), activeSession: active.recordset[0] ? await sessionPayload(memberId, active.recordset[0].id) : null };
}
...
export async function listSchedules(memberId: number, page: number, limit: number, fromDate?: string, toDate?: string, status?: string) {
  ...
  const conditions = ['a.member_id=@memberId'];
  ...
  const [rows, count] = await Promise.all([
    query(`SELECT s.id,s.assignment_id,s.program_day_id,s.scheduled_date,s.status,s.created_at,s.updated_at,a.member_id,a.program_id,a.schedule_timezone,p.name AS program_name,d.week_number,d.day_number,d.title AS day_title FROM dbo.CoachProgramSchedules s JOIN dbo.CoachProgramAssignments a ON a.id=s.assignment_id JOIN dbo.WorkoutPrograms p ON p.id=a.program_id JOIN dbo.WorkoutProgramDays d ON d.id=s.program_day_id WHERE ${where} ORDER BY s.scheduled_date,s.id OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY`, params),
    query(`SELECT COUNT(*) AS total FROM dbo.CoachProgramSchedules s JOIN dbo.CoachProgramAssignments a ON a.id=s.assignment_id WHERE ${where}`, params),
  ]);
...
export async function getSchedule(memberId: number, scheduleId: number) {
  ...
  return { ...schedule, scheduled_date: datePart(schedule.scheduled_date), exercises: exercises.recordset };
}
...
export async function startSession(memberId: number, scheduleId: number) {
  ...
  try {
    const scheduleResult = await new sql.Request(tx).input('memberId', sql.Int, memberId).input('scheduleId', sql.Int, scheduleId).query(`SELECT s.id,s.assignment_id,s.program_day_id,s.scheduled_date,s.status,a.status AS assignment_status,a.schedule_timezone,p.id AS program_id FROM dbo.CoachProgramSchedules s WITH (UPDLOCK,HOLDLOCK) JOIN dbo.CoachProgramAssignments a WITH (UPDLOCK,HOLDLOCK) ON a.id=s.assignment_id AND a.member_id=@memberId JOIN dbo.WorkoutPrograms p ON p.id=a.program_id WHERE s.id=@scheduleId`);
    const schedule = scheduleResult.recordset[0];
    if (!schedule) throw new AppError(404, 'Schedule not found');
    if (schedule.assignment_status !== 'ACTIVE') throw new AppError(409, 'Assignment is not active');
    if (schedule.status !== 'SCHEDULED') throw new AppError(409, 'Schedule is not available to start');
    assertTimeZone(String(schedule.schedule_timezone));
    if (datePart(schedule.scheduled_date) !== todayInTimeZone(String(schedule.schedule_timezone))) throw new AppError(409, 'Only today\'s scheduled workout can be started');

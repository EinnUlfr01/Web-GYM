...
const sessionId = z.object({ memberId: z.coerce.number().int().positive(), sessionId: z.coerce.number().int().positive() }).strict();
...
router.get('/members/:memberId/sessions/:sessionId', validate(sessionId, 'params'), controller.getSession);

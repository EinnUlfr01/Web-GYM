...
async function run() {
  ...
  const progress=await call('GET',`/coach/members/${accounts.memberA.id}/progress`,accounts.coachA); check(progress.status===200 && progress.data.data.blockedReason==='BLOCKED_BY_MEMBER_WORKOUT_FLOW','Coach progress is truthful about missing Member flow');

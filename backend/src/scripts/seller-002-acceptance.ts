...
if(config.db.database==='GYMFIT_DB'||!config.db.database.startsWith('GYMFIT_DB_SELLER002_ACCEPTANCE_'))throw new Error('Refusing mutation outside a disposable SELLER-002 acceptance database');
...
async function call(method:string,path:string,key?:string,body?:unknown){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),15000);const suite=()=>controller.abort();suiteAbort.signal.addEventListener('abort',suite,{once:true});try{const response=await fetch(base+path,{method,signal:controller.signal,headers:{'content-type':'application/json',...(key&&accounts[key]?.token?{authorization:`Bearer ${accounts[key].token}`}:{})},body:body===undefined?undefined:JSON.stringify(body)});lastStatus=response.status;let data:any={};try{data=await response.json();}catch{}return{status:response.status,data};}finally{clearTimeout(timer);suiteAbort.signal.removeEventListener('abort',suite);}}
...
async function main(){
  ...
  const sku=`SELLER002-${stamp}`;r=await expect('POST','/admin/products','admin',201,{product_name:'SELLER-002 admin regression',sku,price:100000,stock:3,category_id:category.id,shop_id:shopA.id});
  const createdProductId=Number(r.data.data.id);const ownership=(await query<any>('SELECT shop_id FROM dbo.Products WHERE id=@id',{id:createdProductId})).recordset[0];check(Number(ownership.shop_id)===Number(official.id),'admin create ignores spoofed shop_id and uses official');
